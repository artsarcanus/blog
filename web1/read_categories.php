...
<div class="wrap">
    <div class="main">
        ...
        <ul>
            ...
            $sql = 'SELECT id,nombre from cms1.categories where id= '.$id.'';
                        foreach ($pdo->query($sql) as $row) {          
                                echo '<li  class="active"><a  href="read_categories.php?id='.$row['id'].'" >'.$row['nombre'].'</a></li>';
                            }Database::disconnect();?>
        </ul>
        <div class="clear"></div>
    </div>
        <div class="content">
                       <?
                       $pdo = Database::connect();
                        $sql = 'SELECT articles.id, articles.title,articles.publicationDate, articles.summary, articles.content, categories.nombre from cms1.articles inner join cms1.categories on articles.categories_id=categories.id where articles.categories_id= '.$id.'';
                        foreach ($pdo->query($sql) as $row) {          
